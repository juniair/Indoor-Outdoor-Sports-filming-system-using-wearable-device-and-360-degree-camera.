...
public class CameraFileListViewActivity extends AppCompatActivity {
    ...
    private final int entryCount = 1; //Parameter value for entryCount (camera.getListFiles)
    ...
    private void setupViews() {
        ...
        //mListView.setVisibility(View.INVISIBLE);            // 비디오 리스트뷰 안보이게 해줌.
...
//            super.onPreExecute();
//            uploading = ProgressDialog.show(CameraFileListViewActivity.this, "Uploading File", "Please wait...", false, false);
//        }
//
//        @Override
...
//            Upload u = new Upload();
//            String msg = u.uploadVideo(Global.selectedPath);
//            return msg;
//        }
//    }
    //
            ...
            commandsExecute.setListener(new HttpAsyncTask.OnHttpListener() {
                ...
                public void onResponse(OnHttpListener.OSCReturnType type, Object response) {
                    if (type == OSCReturnType.SUCCESS) {
                        ...
                        if (Global.GlobalisDeleteFile==false) {     // 모바일로 영상을 다운받는데 성공하면 , 캠에있는 저장물 지움
                            int[] seletedPosi = new int[1];
                            seletedPosi[0] = 0;
                            //deleteFilesFromCamera1(seletedPosi, false);   // 필요없다 생각해서 지움 ( 0번째 파일 캠에서 지우기 )
                            Log.d("deleteFile:: ", "success");
                            Global.GlobalisDeleteFile=true;
                        }
                    } else {
                        Utils.showTextDialog(mContext, getString(R.string.response), Utils.parseString(response));
                    }
...
//            mProgressDialog = new ProgressDialog(CameraFileListViewActivity.this);
//
//            mProgressDialog.setProgressStyle(ProgressDialog.STYLE_HORIZONTAL);
              //mProgressDialog = ProgressDialog.show(mContext, "", "다운로드중입니다...", true, false);
    ...
    private void handleResponse(String fileName, String localUri) {
        ...
        //Update android gallery
            ...
            Global.GlobalisDeleteFile = false;
            Log.d("GlobalisDeleteFile :: ",Global.GlobalisDeleteFile.toString());
            Global.GlobalFileName = fileName;
            Global.selectedPath = "/storage/emulated/0/DCIM/friendsCameraSample/"+Global.GlobalFileName;


            /////   저장하고 finish하면서 다른 wifi-AP 에 접속
            mRunnable = new Runnable() {            //  동영상 저장 후 와이파이 바꿀때 시간간격을 위한 것
                @Override
                public void run() {
...
//                    wifiConfig2.preSharedKey = String.format("\"%s\"", "NGNNGN10");
                    ...
                    Log.d("Global.selectedPath :::",Global.selectedPath);
//                    uploadVideo();
                }
            };

            mHandler = new Handler();
            mHandler.postDelayed(mRunnable, 100); // 에러나면 초 늘리기
...
//        mProgressDialog = ProgressDialog.show(mContext, "", "Processing...", true, false);
    ...
    private void deleteFilesFromCamera1(final int[] positions, final boolean removeAll) {
        String[] fileUrls;
            ...
            public void onResponse(OnHttpListener.OSCReturnType type, Object response) {
                if (type == OSCReturnType.SUCCESS) {
                    ...
                    if (removeAll) {
                        ...
                        adapter.removeItems1(positions);
                    }
                    adapter.notifyDataSetChanged();
                }
